...
app.post('/topup/:id', jsonParser, (req,res) => {
    const user = users.find(u => u.id == parseInt(req.params.id));
    if(!user){
        res.send('The ID was not found')
    }
...
// app.post('/pembayaran', verifyToken, (req, res) =>{
//     jwt.verify(req.token, 'secret', (err, authData) =>{
//         if(err){
//             res.status(403).send("Forbidden")
//         }else{
...
app.post('/pembayaran/:id/:layanan', jsonParser, (req, res) =>{
    const user = users.find(u => u.id == parseInt(req.params.id));
...
app.post('/transfer/:sender/:receiver', jsonParser, (req,res) => {
